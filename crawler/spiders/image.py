...
from crawler.configs import image as config
from scrapy_redis.spiders import RedisSpider

from crawler.items.movie_douban import ImageMovieDouban
...
class ImageSpider(RedisSpider):
    ...
    用法： scrapy crawl image -a type=

    movie_douban        豆瓣电影的图片
    celebrity_douban    豆瓣影人的图片
    ...
    custom_settings = {
        'ITEM_PIPELINES': {
            'crawler.pipelines.image.ImagePipeline': 300
        }
    }
    ...
    def __init__(self, type=None, **kwargs):
        ...
        self.type = type
        self.type_movie = 'movie_douban'
        self.type_celebrity = 'celebrity_douban'
    ...
    def parse(self, response):
        ...
        if image_list:
            for index, image in enumerate(image_list):
                ...
                item_image['id'] = re.search('photo/(\d+)', image.xpath('.//div[@class="cover"]/a/@href').get()).group(1)
                if self.type == self.type_movie:
                ...
                print(item_image)
                yield item_image
            self.logger.info('get image success,id:{},type:{}'.format(id, self.type))
        else:
            self.logger.warning('get image failed,id:{},type:{}'.format(id, self.type))
