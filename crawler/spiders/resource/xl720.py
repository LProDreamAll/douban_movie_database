...
class Xl720ResourceSpider(BaseSpider):
    ...
    def parse_movie_list(self, response):
        ...
        if movie_list:
                ...
                if url is not None:
                    ...
                    if title is not None:
                        yield scrapy.Request(url=url,
                                             meta={'movie_id': movie_id, 'title': title.group(1),
                                                   'year': title.group(2)},
                                             callback=self.parse_movie, priority=2)
            self.logger.info(
            ...
            if self.type == self.type_new and page_id > self.new_max_pages:
                return
            # 下一页
            yield scrapy.Request(url='{}/filter/page/{}'.format(config.URL_XL720, page_id + 1),
                                 meta={'page_id': page_id + 1},
                                 callback=self.parse_movie_list, priority=1)
        else:
            self.logger.error(
    ...
    def parse_movie(self, response):
        ...
        resource_list = response.xpath('//div[@class="download-link"]/a') + response.xpath('//div[@class="ztxt"]/a')
        if resource_list:
            imdb_id = 0
            ...
            self.logger.error('get xl720\'s movie failed,movie_id:{},movie_name:{}'.format(movie_id, title))
